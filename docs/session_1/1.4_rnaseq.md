...
!!! example "Create a new working directory"
    ...
    You can open the new directory either via the VS Code GUI (`File > Open Folder...` or `Ctrl + K Ctrl + O` / `Cmd + O`) and navigating to it, or by using the `code` command in the terminal:
...
:eyes: Looking at the `nf-core/rnaseq` pipeline structure provided in the [introduction](https://nf-co.re/rnaseq/3.23.0), we can see that the developers have:
...
Our input FASTQ files (`fastq/`), reference data (`mm10_reference/`), and full sample sheet (`samplesheet.csv`) are already available within the home directory. Take a look at the files.
