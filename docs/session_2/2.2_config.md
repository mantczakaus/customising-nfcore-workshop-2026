...
### Using a custom configuration profile
...
As we continue customising our run on the workshop VMs, it makes sense to **define a profile**. 
...
To apply this profile to our run, we would now include both the profile name and the config file on the Nextflow run command:

```bash
nextflow run rnaseq/main.nf -profile singularity,workshop -c nectar_vm.config ...
```
<br>

We can simplify this slightly by dropping the singularity profile from the run command and including it within our 'workshop' profile. We can do this using the [`singularity scope`](https://docs.seqera.io/nextflow/reference/config#singularity).
...
!!! example "Exercise 2.2.4.2 :stopwatch: 4 mins"
    ...
    Edit `nectar_vm.config` to enable the use of singularity for our run within the `singularity scope`. 
...
![](../assets/2.2_selector_priority.png){width=70%}
