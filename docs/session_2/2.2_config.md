...
!!! example "Exercise 2.2.2.2 :stopwatch: 2 mins"
    ...
    How can we use singularity for software access, rather than the default of locally installed tools established in `conf/base.config`? 
...
!!! example "Exercise 2.2.2.3 :stopwatch: 3 mins"
    ...
    ??? success "Solution"
        ...
        The process label can then be extracted from the `modules/nf-core/star/align/main.nf` file, for example with `grep`:
...
!!! tip "Bottleneck processes"

    A 'bottleneck process' is one that requires more walltime than others, and holds up the entire pipeline as downstream processes rely on its outputs. For our workshop VMs and tiny test data, this doesn't mean much, but in real-world applications with full-size data and potentially large sample cohorts, ensuring that processes are not provided more CPU or RAM than they need can help analyse your data faster. 
...
Either the trace text file or the html run report are a good place to review usage and get a sense of how your pipeline is running on your computer. 
...
!!! example "Exercise 2.2.5.1 :stopwatch: 3 mins"
    ...
    ??? success "Solution"
        ...
        On the figure for 'Memory', all processes except STAR_ALIGN are using near to or less than 1 GB RAM. This indicates that for our workshop data, most processes could execute with less RAM and custom resource customisation is warranted.
...
We can use wildcards (`*`) and 'or' (`|`) notation with `withLabel` to apply the same configuration to more than one process label. 
...
!!! example "Exercise 2.2.6.1 :stopwatch: 5 mins"
    ...
    ??? hint Hint
        ...
        From a previosuly completed run, you can obtain the process execution path from the execution trace, timeline or report files within `<outdir>/pipeline_info`.

        For example using the execution trace file:
    ...
    ??? hint Hint
        Follow the same general syntax as `withLabel`, including enclosing the STAR_ALIGN execution path in single quotes
...
!!! example "Exercise 2.2.6.3 :stopwatch: 2 mins"
    ...
    You should now see two STAR processes running at once instead of one, and 🤞 the run completes faster!


<br>

⏲️ If you review the process execution times within the `pipeline_info` files, you'll see that even our bottleneck process completed in < 2 minutes, with most others requiring only seconds. For this demonstration, it is therefore unlikely that our run will complete much faster following our customisations for this small dataset on these workshop VMs. 
