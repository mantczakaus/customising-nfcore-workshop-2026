...
To apply this profile to our run, we would now include both the profile name and the workshop profile config file on the Nextflow run command:
...
We can simplify this slightly by dropping the singularity profile from the run command and including it within our institutional config. We can do this using the [`singularity scope`](https://docs.seqera.io/nextflow/reference/config#singularity).
